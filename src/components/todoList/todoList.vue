	...
	<div class="todo-list" id="todo-list">
		...
		<draggable v-model="todos" @start="drag=true" @end="drag=false" >
			<div v-for="(todo, key) in todos" :key="key">
				<div class="todo-item">
					<input type="checkbox" @click="clickTodo(key)" :checked="todos[key].completed">
					<input v-model="todos[key].content" @keydown="editTodo(key)" @keyup.enter="completeTodo(key)" :class="{ 'completed': todos[key].completed }" >
...
export default {
  ...
  methods: {
    addTodo: function() {
      ...
      this.$http
        .post("https://mynote-example.firebaseio.com/todolist.json", {
    ...
    removeTodo(id) {
      ...
      for (let id in this.todos) {
        this.$http.post(
          "https://mynote-example.firebaseio.com/todolist.json",
          this.todos[id]
        );
      }
    },
  ...
  created() {
    this.$http
      .get("https://mynote-example.firebaseio.com/todolist.json")
      .then(function(data) {
        if (!data.body) {
          this.todos = [];
        } else {
